...
function encodePwd(u_id, pwd, verifycode) {
    u_id = uin2hex(u_id);
    return $.Encryption.getEncryption(pwd, u_id, verifycode, undefined);
}

function cdata(randstr, ans, M) {
    for (var a = 0; a < M && a < 1e3; a++) {
        var s = randstr + a,
            c = $.Encryption.md5(s);
